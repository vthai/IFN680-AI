...
class ProbDist:
    ...
    def show_approx(self, numfmt='%.4g'):
        """Show the probabilities rounded and sorted by key, for the
        sake of portable doctests."""
        return ', '.join([('%s: ' + numfmt) % (v, p)
                          for (v, p) in sorted(self.prob.items())])

    #Y. Xu added, to remove an item 
    ...
    #Y. Xu added, to check whether the probability distribution is valid or not,
    #             i.e., whether the sum of the probabilities is 1
    def is_valid(self):
